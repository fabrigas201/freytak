...
<tr valign="top">
	...
	<table class="contTbl">
		...
		{/if}
		{if $smarty.get.mod == 'pages'}
		<tr>
			<td valign="top">Теги</td>
			<td >
				<div style="max-height:250px; overflow-y:hidden;" class="">
				{if $vars.tagsSystem|count > 0}
				{foreach $vars.tagsSystem as $tgsSys}
					<label><input {if isset($tagsForPage[$tgsSys -> id] -> tgs_id) &&$tagsForPage[$tgsSys -> id] -> tgs_id == $tgsSys -> id} checked {/if} type="checkbox" name="tgs_id[]" value="{$tgsSys -> id}"/> {$tgsSys -> title}</label> <br />
				{/foreach}
				{/if}
				</div>
			</td>
